from torchmetrics.image.fid import FrechetInceptionDistance
import torch
import csv
import os
from PIL import Image
import numpy as np


def load_images_as_tensor(folder, image_size=299):
    images = []
    for fname in os.listdir(folder):
        if fname.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
            path = os.path.join(folder, fname)
            try:
                img = Image.open(path).convert("RGB").resize((image_size, image_size))
                img = torch.from_numpy(np.array(img)).permute(2, 0, 1).float() / 255.0
                images.append(img)
            except Exception as e:
                print(f"Error loading {path}: {e}")
    if images:
        return torch.stack(images)
    else:
        return torch.empty((0, 3, image_size, image_size))


real_images = "/home/data/yunsu/SCoFT/culture_data/korea/train/images"
fake_images = ["/home/evaluation/face",
                "/home/evaluation/GQA",
                "/home/evaluation/merged",
                "/home/evaluation/vanila"]

fid_results = []
real_imgs = load_images_as_tensor(real_images)
for fake_dir in fake_images:
    fake_imgs = load_images_as_tensor(fake_dir)
    fid = FrechetInceptionDistance(normalize=True)
    fid.update(real_imgs, real=True)
    fid.update(fake_imgs, real=False)
    fid_score = float(fid.compute())
    print(f"FID for {fake_dir}: {fid_score}")
    fid_results.append({"model": os.path.basename(fake_dir), "fid_score": fid_score})

# Save results to CSV
csv_path = "fid_results.csv"
with open(csv_path, "w", newline="", encoding="utf-8") as csvfile:
    fieldnames = ["model", "fid_score"]
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()
    writer.writerows(fid_results)
print(f"Saved FID results to {csv_path}")
